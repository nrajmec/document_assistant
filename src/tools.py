...
def create_calculator_tool(logger: ToolLogger):
    ...
    def calculator(expression: str) -> str:
        ...
        Args:
            expression: A mathematical expression using numbers and the
                operators +, -, *, /, %, ** and parentheses
                (e.g. '1250.50 + 340.25', '(100 - 20) * 3').

        Returns:
            A formatted string with the calculation result, or an error
            message if the expression is invalid or unsafe.
        """
        try:
            if not re.fullmatch(r"[0-9\s\.\+\-\*/%\(\)]+", expression):
                error_msg = f"Invalid expression: '{expression}' contains disallowed characters."
                logger.log_tool_use(
                    "calculator",
